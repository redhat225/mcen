...
class UsersTable extends Table
{
    ...
    public function validationDefault(Validator $validator){
        ...
        $validator
            ->scalar('user_personal_email')
            ->maxLength('user_personal_email', 60)
            ->requirePresence('user_personal_email', 'create')
            ->notEmpty('user_personal_email');

        $validator
            ->scalar('user_pro_email')
            ->maxLength('user_pro_email', 60)
            ->requirePresence('user_pro_email', 'create')
            ->notEmpty('user_pro_email');
   ...
   public function beforeMarshal(Event $event, ArrayObject $data, ArrayObject $options){
    ...
    {
        $rules->add($rules->isUnique(['user_pro_email']));
        $rules->add($rules->isUnique(['user_personal_email']));
        $rules->add($rules->isUnique(['user_contact']));
        return $rules;
    }
